...
$query_con = $pdo->query("SELECT * FROM caixa WHERE operador = '$id_usuario' and status_caixa = 'Aberto'");
...
if ($total_reg > 0) {
	...
	if ($desconto_porcentagem == 'Sim') {
		...
		$desconto = str_replace(',', '.', $desconto); // Substitui vírgulas por ponto, se houver
		$desconto = preg_replace('/[^0-9.]/', '', $desconto); // Remove caracteres não numéricos
		$desconto = is_numeric($desconto) ? (float)$desconto : 0; // Garante que seja um número ou define 0
	...
	if (!is_numeric($valor_total)) {
		$valor_total = 0; // Valor padrão caso seja inválido
	}
	$valor_total = number_format($valor_total, 2, ',', '.');;
	if (!is_numeric($valor)) {
		$valor = 0; // Valor padrão caso seja inválido
	}
